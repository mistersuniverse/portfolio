    ...
    <div>
      ...
      <div className='flex justify-evenly items-center flex-wrap mt-16 mx-8 4xl:py-4'>
        {
          [ 'UI/UX', 'Web App', 'Mobile App', 'React App', 'All' ].map((item, index) => (
            <div key={index} onClick={() => handleWorkFilter(item)} className={`${ activeFilter === item && 'bg-secondary text-white' } py-2 px-4 rounded-lg text-black cursor-pointer font-extrabold hover:bg-secondary hover:text-white 4xl:py-4 4xl:px-8 4xl:rounded-xl`}>
              {item}
            </div>
          ))
        }
      ...
      >
        { 
          ...
          <div className='w-full m-4 xs:w-72 4xl:w-[470px] flex-col xs:m-8 p-4 4xl:p-5 4xl:rounded-xl rounded-lg bg-white text-black cursor-pointer hover:shadow-2xl shadow-lg' key={index}> 
            <div className='w-full h-56 relative'>
              <img className='w-full h-full rounded-lg object-cover 4xl:h-[350px]' src={urlFor(work.imgUrl)} alt={work.title} />

              <motion.div
                whileHover={{ opacity: [ 0, 1]}}
                transition={ { duration: 0.25, ease: 'easeOut', staggerChildren: 0.5}}
                className='absolute top-0 left-0 bottom-0 right-0 w-full h-full bg-opacity-50 rounded-lg opacity-0'
              >
                <a className='w-12 h-12 flex' href={work.projectLink} target='_blank' rel='noreferr'>
                  <motion.div
                ...
                <a className='w-12 h-12 flex' href={work.codeLink} target='_blank' rel='noreferr'>
                  <motion.div
                    whileInView={{ scale: [ 0, 1] }}
                    whileHover={{ scale: [1, 0.9] }}
                    transition={ { duration: 0.25 }}
            ...
            <div className='p-2 w-full relative flex flex-col items-center justify-center'>
              ...
              <div className='absolute py-1 px-4 rounded-lg bg-white top-[-25px] left-[70px]'>
