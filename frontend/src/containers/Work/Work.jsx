...
const Work = () => {
  ...
  return (
    <div>
      ...
      <div className='flex justify-center items-center flex-wrap mt-16 mx-8 4xl:py-4 gap-2'>
        {
          [ 'vanilla', 'mern', 'react', 'all' ].map((item, index) => (
            <div  key={index} onClick={() => handleWorkFilter(item)} className={`${ activeFilter === item && 'bg-secondary text-white' } py-2 px-4 rounded-lg text-black cursor-pointer font-bold font-base hover:bg-secondary hover:text-white 4xl:py-4 4xl:px-8 4xl:rounded-xl uppercase`}>
              {item}
            </div>
          ))
        }
      ...
      { filterWork.length ? (
          ...
          > 
            ...
            animate={animateCard}
            transition={{ duration: 0.5, delayChildren: 0.5 }}
            className='flex flex-wrap justify-center items-center mt-24'
          >
              no projects to show
            </motion.div>
          )}
